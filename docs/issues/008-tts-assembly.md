...
- [x] 全应用无 Fake 残留，真机跑通「导入 → 通览 → 回忆 → 考察 → 次日复习」闭环（簿记回填 2026-09-16：下方 09-15 补记已验并关单，此处未勾属遗漏；后续 9f6518b~7852c35 六轮补丁未动会话主链路，用户日常实机使用持续覆盖）
- [ ] 断网时：缓存命中正常播报、未命中静默跳过且流程不阻断（09-15 冒烟已验；7852c35 播放层换 WAV 分流后留一项增量回归：断网下含 WAV 缓存字节（如 P）拼报正常 + 生词静默跳过不阻断）

## 验收补记 + 关单（2026-09-15）

- 补丁链：0009（d60329a TTS 真实现）→ 0010（8ca20e2 复审反馈：重听放行闭包防 hold 泄漏 / 取消分支 flush 防残音 / 清死代码）→ 0011（acd82be 用户裁定：听写英文框在上中文在下 + 初始焦点英文框，PRD 修订 #26）。
- 用户实机冒烟（20:00）：核心必验（播报中提交不中断、批改回放隐式顶掉、计时无跳变）、计时联动（播报结束起计/断网立即起计）、音色（中文女声 type=1、英文美音）、告警音时序、两级缓存断网行为、重听回归——**全部通过**；反馈 2 条均为需求裁定非缺陷（拼写回放提议同日撤回，维持 PRD 口径）。
- **ISSUE-008 关单**（0009+0010+0011 三补丁，main 至 acd82be）。遗留观察项：无。
